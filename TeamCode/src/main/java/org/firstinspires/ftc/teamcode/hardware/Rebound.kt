 ...
 * @param parentOpMode many functions need an OpMode to run
 ...
 */
class Rebound(private val parentOpMode: OpMode,
              ...
              private var imu: BNO055IMU)
{
    ...
    fun initialize(motorMode: DcMotor.RunMode = DcMotor.RunMode.RUN_WITHOUT_ENCODER,
        ...
        // initialize the IMU
        imu = parentOpMode.hardwareMap.get(BNO055IMU::class.java, "imu")
        // apply IMU configuration
        imu.initialize(parameters)
        // basic IMU tests
        if (imu.isGyroCalibrated) {
            parentOpMode.telemetry.addLine("IMU Gyro is calibrated. AngleSnap should work properly!")
        }
    }
     ...
     */
    fun stopAndResetEncoders(mode: DcMotor.RunMode = DcMotor.RunMode.RUN_USING_ENCODER) {
        stopMoving()
    ...
    */
    ...
    fun arcadeMecanum_WPI(vtX: Double, vtY: Double, vR: Double) {
        ...
        parentOpMode.telemetry.addData("Inputs: ", listOf(vD, thetaD, vTheta))
        ...
        parentOpMode.telemetry.addData("Pre-Scale: ", listOf(lfPow, rfPow, lbPow, rbPow))
        ...
        parentOpMode.telemetry.addData("Scaled: ", listOf(lfPow, rfPow, lbPow, rbPow))
        lf.power = lfPow
        rf.power = rfPow
        lb.power = lbPow
        rb.power = rbPow
     ...
     * @return TRUE if ALL motors is busy, FALSE if any motor is not.
     */
     ...
     */
    fun stopMoving(float: Boolean = true) {
        if (float) {setZeroPowerBehaviors(DcMotor.ZeroPowerBehavior.BRAKE)}
        lf.power = 0.0
        lb.power = 0.0
        rf.power = 0.0
        rb.power = 0.0
        if (float) {setZeroPowerBehaviors(DcMotor.ZeroPowerBehavior.FLOAT)}
    }
    /*
